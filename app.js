...
async function processUserMessage(phoneNumber, message, mediaData = null) {
   try {
     ...
     if (mediaData && mediaData.data) {
       ...
       if (currentStep === 'predial' || currentStep === 'certificado_libertad') {
         ...
         try {
           ...
           let validation;
           try {
             console.log(`🤖 Intentando validación con Claude para ${currentStep}`);
             validation = await documentValidator.validateDocument(
               mediaData.data, 
               currentStep, 
               mediaData.mimetype
             );
             console.log(`✅ Validación con Claude exitosa`);
           } catch (claudeError) {
     ...
     if (response.type === 'final_confirmation') {
       ...
       // Enviar email y registrar en Google Sheets de forma asíncrona
       setTimeout(async () => {
         try {
           ...
           const sheetsResult = await sheetsService.addPropertyToSheet(finalPropertyData);
           if (sheetsResult.success) {
             console.log(`📊 Propiedad registrada en Google Sheets: ${sheetsResult.updatedRows} filas actualizadas`);
           } else {
             console.error(`❌ Error registrando en Google Sheets: ${sheetsResult.error}`);
           }
         } catch (error) {
           console.error('❌ Error en proceso post-completado:', error);
         }
       }, 2000);
 ...
 try {
   ...
   if (confirmed) {
     ...
     const finalMessage = `✅ ¡Perfecto! Tu propiedad ha sido registrada exitosamente.\n\n` +
       ...
       // Enviar email y Google Sheets de notificación
       setTimeout(async () => {
         try {
           ...
           // Google Sheets
           const sheetsResult = await sheetsService.addPropertyToSheet(updatedData);
           if (sheetsResult.success) {
             console.log(`📊 Propiedad registrada en Google Sheets desde confirmación manual`);
           }
         } catch (error) {
  ...
  app.post('/admin/complete-property/:phone', async (req, res) => {
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          // Google Sheets
          const sheetsResult = await sheetsService.addPropertyToSheet(finalData);
          if (sheetsResult.success) {
            console.log(`📊 Propiedad de testing registrada en Google Sheets`);
          }
        } catch (error) {
          console.error('Error enviando notificaciones de testing:', error);
        }
      }, 1000);
   ...
   app.get('/admin/test-sheets', async (req, res) => {
    try {
      ...
      res.json({
        ...
        message: testResult.success ? 'Conexión exitosa con Google Sheets' : 'Error de conexión',
        spreadsheetId: sheetsService.spreadsheetId,
        sheetInfo: testResult.success ? testResult : null,
        error: testResult.error || null
      });
      
    } catch (error) {
      console.error('Error probando Google Sheets:', error);
      res.status(500).json({ error: 'Error probando conexión con Google Sheets' });
    }
   });
   
   // Endpoint para crear headers en Google Sheets
   app.post('/admin/create-sheets-headers', async (req, res) => {
    try {
      console.log('📊 Creando headers en Google Sheets...');
      
      const result = await sheetsService.createHeaders();
      
      res.json({
        success: result.success,
        message: result.success ? 'Headers creados exitosamente' : 'Error creando headers',
        spreadsheetId: sheetsService.spreadsheetId,
      ...
      res.status(500).json({ error: 'Error creando headers en Google Sheets' });
    }
   });
   
   // Endpoint para probar registro manual en Google Sheets
   app.post('/admin/test-sheets-insert', async (req, res) => {
    try {
      const { phoneNumber } = req.body;
      ...
      console.log(`📊 Probando inserción en Google Sheets para ${phoneNumber}`);
      
      const result = await sheetsService.addPropertyToSheet(propertyData);
      
      res.json({
        success: result.success,
        message: result.success ? 'Propiedad registrada en Google Sheets' : 'Error registrando propiedad',
        result: result,
        propertyId: propertyData.id
      });
      ...
      res.status(500).json({ error: 'Error probando inserción en Google Sheets' });
    }
   });
   ...
   app.get('/health', async (req, res) => {
    try {
      ...
      const services = {
        ...
        sheets: {
          configured: true,
          ready: await sheetsService.testConnection().then(r => r.success).catch(() => false)
        }
      };
   ...
   app.listen(PORT, () => {
    ...
    console.log(`📊 Test Google Sheets: GET ${PORT}/admin/test-sheets`);
    console.log(`📋 Crear headers Sheets: POST ${PORT}/admin/create-sheets-headers`);
    console.log(`📊 Test inserción Sheets: POST ${PORT}/admin/test-sheets-insert`);
    console.log('');
    ...
    console.log(`📊 Google Sheets: Verificando...`);
    console.log('='.repeat(60));
